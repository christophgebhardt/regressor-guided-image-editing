...
def rgb2hsl_torch(rgb: torch.Tensor) -> torch.Tensor:
    hsl_h, cmin, cmax, delta = rgb2hs(rgb)
    hsl_l = (cmax + cmin) / 2.
...
def rgb2hs(rgb):
    ...
    delta = cmax - cmin
    hs_h = torch.empty_like(rgb[:, 0:1, :, :])
    cmax_idx[delta == 0] = 3
    hs_h[cmax_idx == 0] = (((rgb[:, 1:2] - rgb[:, 2:3]) / delta) % 6)[cmax_idx == 0]
    hs_h[cmax_idx == 1] = (((rgb[:, 2:3] - rgb[:, 0:1]) / delta) + 2)[cmax_idx == 1]
    hs_h[cmax_idx == 2] = (((rgb[:, 0:1] - rgb[:, 1:2]) / delta) + 4)[cmax_idx == 2]
    hs_h[cmax_idx == 3] = 0.
    hs_h /= 6.
    return hs_h, cmin, cmax, delta
...
def hsv2rgb_torch(hsv: torch.Tensor) -> torch.Tensor:
    ...
    return hslv2rgb(rgb, idx, _c, _x, _o, _m)
...
def hslv2rgb(rgb, idx, _c, _x, _o, _m):
